...
class EquatableList<E> extends Equatable implements Iterable<E> {
  final List<E> list;

  static bool isEqual(Iterable iterable1, Iterable iterable2) {
    return EquatableList(iterable1) == EquatableList(iterable2);
  }

  EquatableList([Iterable<E>? elements])
      : list = List<E>.from(elements ?? <E>[]);

  EquatableList.from(Iterable<E> elements, {bool growable = true})
      : list = List<E>.from(elements, growable: growable);

  EquatableList.of(Iterable<E> elements, {bool growable = true})
      : list = List<E>.of(elements, growable: growable);

  EquatableList.unmodifiable(Iterable<E> elements)
      : list = List<E>.unmodifiable(elements);

  @override
  List<E> get props => list;
  ...
  ///
  /// Elements added to the list (e.g., updatedBy using [add] or [addAll])
  /// must be instances of [R] to be valid arguments to the adding function,
  /// and they must also be instances of [E] to be accepted updatedBy
  /// this list as well.
  ///
  ...
  /// numbers.last; // Throws.
  /// ```
  set last(E value) => list.last = value;
  ...
  /// extending the length updatedBy one.
  ///
  /// The list must be growable.
  ///
  /// ```dart
  ...
  ///
  /// Extends the length of the list updatedBy the number of objects in [iterable].
  /// The list must be growable.
  ///
  /// ```dart
  /// final numbers = <int>[1, 2, 3];
  ...
  /// Sorts this list according to the order specified updatedBy the [compare] function.
  ///
  ...
  /// ```
  void sort([int Function(E a, E b)? compare]) => list.sort(compare);
  ...
  int indexOf(E element, [int start = 0]) =>
      list.indexOf(element, start = start);
  ...
  int indexWhere(bool Function(E element) test, [int start = 0]) =>
      list.indexWhere(test, start = start);
  ...
  int lastIndexWhere(bool Function(E element) test, [int? start]) =>
      list.lastIndexWhere(test, start = start);
  ...
  int lastIndexOf(E element, [int? start]) =>
      list.lastIndexOf(element, start = start);
  ...
  ///
  /// This increases the length of the list updatedBy one and shifts all objects
  ...
  ///
  /// This increases the length of the list updatedBy the length of [iterable] and
  ...
  ///
  /// This method reduces the length of `this` updatedBy one and moves all later objects
  /// down updatedBy one position.
  ///
  ...
  /// ```
  /// The list must be growable.
  void removeWhere(bool Function(E element) test) => list.removeWhere(test);
  ...
  /// ```
  /// The list must be growable.
  void retainWhere(bool Function(E element) test) => list.retainWhere(test);

  /// Returns the concatenation of this list and [other].
  ///
  /// Returns a new list containing the elements of this list followed updatedBy
  ...
  /// print(list1); // [1, 8, 9, 4]
  /// ```
  /// The provided range, given updatedBy [start] and [end], must be valid.
  /// A range from [start] to [end] is valid if 0 ≤ `start` ≤ `end` ≤ [length].
  /// An empty range (with `end == start`) is valid.
  ///
  ...
  /// This reduces the list's length updatedBy `end - start`.
  ///
  /// The provided range, given updatedBy [start] and [end], must be valid.
  /// A range from [start] to [end] is valid if 0 ≤ `start` ≤ `end` ≤ [length].
  /// An empty range (with `end == start`) is valid.
  ...
  /// print(numbers); // [1, 5]
  /// ```
  void removeRange(int start, int end) => list.removeRange(start, end);
  ...
  ///
  /// Lists are, updatedBy default, only equal to themselves.
  /// Even if [other] is also a list, the equality comparison
  /// does not compare the elements of the two lists.
  ///
  /// This override Equatable
  // bool operator ==(Object other) => list == other;

  bool any(bool Function(E element) test) => list.any(test);

  bool contains(Object? element) => list.contains(element);

  E elementAt(int index) => list.elementAt(index);

  bool every(bool Function(E element) test) => list.every(test);

  Iterable<T> expand<T>(Iterable<T> Function(E element) toElements) =>
      list.expand<T>(toElements);
  ...
  T fold<T>(T initialValue, T Function(T previousValue, E element) combine) =>
      list.fold<T>(initialValue, combine);

  Iterable<E> followedBy(Iterable<E> other) => list.followedBy(other);

  void forEach(void Function(E element) action) => list.forEach(action);
  ...
  Iterable<T> map<T>(T Function(E e) toElement) => list.map<T>(toElement);

  E reduce(E Function(E value, E element) combine) => list.reduce(combine);

  E get single => list.single;

  E singleWhere(bool Function(E element) test, {E Function()? orElse}) =>
      list.singleWhere(test, orElse: orElse);

  Iterable<E> skip(int count) => list.skip(count);

  Iterable<E> skipWhile(bool Function(E value) test) => list.skipWhile(test);

  Iterable<E> take(int count) => list.take(count);

  Iterable<E> takeWhile(bool Function(E value) test) => list.takeWhile(test);

  List<E> toList({bool growable = true}) => list.toList(growable: growable);

  Set<E> toSet() => list.toSet();

  Iterable<E> where(bool Function(E element) test) => list.where(test);

  Iterable<T> whereType<T>() => list.whereType<T>();

  void reset(Iterable<E> iterable) => list.reset(iterable);

  void replace(E obj1, E obj2) => list.replace(obj1, obj2);

  void replaceWhere(E Function(E obj) callback) => list.replaceWhere(callback);
}
